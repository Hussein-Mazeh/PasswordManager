...
const scrapedCredentialCache = new Map();
...
function getSafeBrowsingSetting() {
...
async function readSafeBrowsingEnabled() {
...
async function setSafeBrowsingEnabledTrue() {
    const setting = getSafeBrowsingSetting();
    if (!setting) {
        return true;
    }
...
async function ensureSafeBrowsingEnabled() {
...
async function showSafeBrowsingWarning(message) {
    if (!chrome.notifications?.create) {
        console.warn("PassMan Safe Browsing →", message);
        return;
    }
    return new Promise((resolve) => {
        chrome.notifications.create(SAFE_BROWSING_NOTIFICATION_ID, {
            type: "basic",
...
async function handleSafeBrowsingDisabled() {
    const restored = await setSafeBrowsingEnabledTrue();
    if (restored) {
        await showSafeBrowsingWarning("Chrome Safe Browsing is required for PassMan. It was turned off and has been re-enabled.");
    }
    else {
        await showSafeBrowsingWarning("Chrome Safe Browsing is required for PassMan, but it could not be re-enabled automatically. Please enable it.");
    }
}
function initializeSafeBrowsingEnforcement() {
    const setting = getSafeBrowsingSetting();
    if (!setting) {
        return;
    }
    ...
    void ensureSafeBrowsingEnabled().then((enabled) => {
        if (!enabled) {
            void showSafeBrowsingWarning("Chrome Safe Browsing must be enabled for PassMan to operate. Please enable it in Chrome settings.");
        }
    });
}
...
function setPhishingAlert(tabId, reasons) {
...
function clearPhishingAlert(tabId) {
    void chrome.action.setBadgeText({ text: "" });
    if (tabId === undefined) {
        return;
    }
        ...
        switch (message.type) {
            ...
            case "UNLOCK": {
                try {
                    ...
                    const code = err?.code || "UNLOCK_FAILED";
                    console.error("PassMan UNLOCK → failed", code, err);
                    sendResponse({ ok: false, code, message: err.message });
                }
                finally {
                    if (typeof message.masterPassword === "string")
                        message.masterPassword = "";
                }
                return;
            }
            ...
            case "REQUEST_FILL": {
                ...
                try {
                    ...
                    if (!safeBrowsingOk) {
                        await showSafeBrowsingWarning("Chrome Safe Browsing must remain enabled for PassMan to autofill passwords.");
            ...
            case "SAVE_CREDENTIAL": {
                ...
                try {
                    ...
                    const safeBrowsingOk = await ensureSafeBrowsingEnabled();
                    if (!safeBrowsingOk) {
                        await showSafeBrowsingWarning("Chrome Safe Browsing must remain enabled for PassMan to save credentials.");
                        sendResponse({ ok: false, code: "SAFE_BROWSING_REQUIRED" });
                        return;
                    }
